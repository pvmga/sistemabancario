...
@section('content')
    <form action="{{ route('editUserSubmit') }}" method="post" novalidate>
        ...
        <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-sm text-center">
            ...
            <input name="name" type="text" placeholder="Nome completo" class="w-full mb-4 p-2 border rounded" value="{{ $user->name }}" />
            <input name="email" type="email" placeholder="Email" class="w-full mb-4 p-2 border rounded" value="{{ $user->email }}" />
            <input name="password" type="password" placeholder="Senha" class="w-full mb-4 p-2 border rounded" value="{{ $user->password }}" />
            <input name="password_confirmation" type="password" placeholder="Confirmar Senha" class="w-full mb-6 p-2 border rounded" value="{{ $user->password }}" />
